#!/bin/bash
# DreamZero-Lite: Wan2.1-T2V-1.3B Full Fine-tune on DROID
# 200K steps, bs=8/GPU × 8 GPUs = 64 global batch
# Equivalent to paper's 100K steps × bs=128
# ETA: ~10 hours on 8xH100
# Resume: auto-detects checkpoints in output_dir
# W&B: dreamzero-lite project
set -euo pipefail
...
echo "[$(date)] Starting DreamZero-Lite training..."
echo "  To stop gracefully: Ctrl+C twice"
...
for ATTEMPT in $(seq 1 $MAX_RETRIES); do
    ...
    output_dir=/mnt/localssd/dreamzero/checkpoints/wan21_1_3b_full_200k \
    ...
    dit_version=/mnt/localssd/dreamzero/checkpoints/Wan2.1-T2V-1.3B \
    text_encoder_pretrained_path=/mnt/localssd/dreamzero/checkpoints/Wan2.1-T2V-1.3B/models_t5_umt5-xxl-enc-bf16.pth \
    ...
    vae_pretrained_path=/mnt/localssd/dreamzero/checkpoints/Wan2.1-T2V-1.3B/Wan2.1_VAE.pth \
